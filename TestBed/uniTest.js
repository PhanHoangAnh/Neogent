          ...
          for (var i = 0; i < atts.length; i++) {
              with({
                  ...
                  if (aggregateObj[atts[n]["attributes"]["sysId"]].indexOf(atts[n]["InputValue"]) == -1) {
                      var flattenArr = []
                      if (aggregateObj[atts[n]["attributes"]["sysId"]] instanceof Array) {
                          flattenArr = [].concat.apply([], atts[n]["InputValue"]);
                      } else {
                          flattenArr.push(atts[n]["InputValue"]);
                      }

                      console.log("flatten Array: ", flattenArr);
                      aggregateObj[atts[n]["attributes"]["sysId"]].concat(flattenArr.filter(function (item) {
